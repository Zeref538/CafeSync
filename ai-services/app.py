...
class CafeSyncAI:
    def __init__(self):
        ...
        # Bean and Beyond location: Caloocan City, Philippines
        self.cafe_location = {
    ...
    def create_models(self):
        ...
        time_effect = np.where(
            ...
            np.where(
                (hour >= 12) & (hour <= 14),
                1.3,  # Lunch
                np.where((hour >= 17) & (hour <= 19), 1.2, 1.0),
            ),
        )  # Evening

        demand = base_demand * weather_effect * time_effect + np.random.normal(
            0, 10, n_samples
        )
    ...
    def get_recommendations(self, user_id, order_history):
        ...
        for item in popular_items:
            pref_types = preferences.get("preferred_types", [])
            if any(pref in item["name"].lower() for pref in pref_types):
                item["score"] += 0.2
    ...
    def optimize_inventory(self, current_inventory, predicted_demand):
        ...
        for item in current_inventory:
            ...
            predicted_usage = predicted_demand * 0.1  # Simplified
        ...
        reorder_count = len([r for r in recommendations if r["action"] == "reorder"])
        # Simplified cost calculation
        estimated_cost = sum(r.get("quantity", 0) * 5 for r in recommendations)

        return {
            "recommendations": recommendations,
            "total_items_to_reorder": reorder_count,
            "estimated_cost": estimated_cost,
        }

    def get_weather_for_cafe(self):
        """Fetch weather data for Bean and Beyond location"""
        try:
            if self.weather_api_key == "demo-key":
                ...
                return {
                    ...
                    "location": self.cafe_location,
                }

            # Fetch real weather from OpenWeatherMap API
            url = "https://api.openweathermap.org/data/2.5/weather"
            params = {
                "lat": self.cafe_location["lat"],
                "lon": self.cafe_location["lon"],
...
def get_recommendations(user_id):
    try:
        ...
        recs = ai_service.get_recommendations(user_id, order_history)
        return jsonify(recs)
...
def optimize_inventory():
    try:
        ...
        opt = ai_service.optimize_inventory(current_inventory, predicted_demand)
        return jsonify(opt)
...
def analyze_sales():
    try:
        ...
        analysis = {
            ...
            "trends": {"growth_rate": 0.12, "seasonality": "moderate"},  # Simplified
        }
...
@app.route("/weather/cafe", methods=["GET"])
def get_cafe_weather():
    """Get current weather for Bean and Beyond cafe location"""
    try:
        weather_data = ai_service.get_weather_for_cafe()
